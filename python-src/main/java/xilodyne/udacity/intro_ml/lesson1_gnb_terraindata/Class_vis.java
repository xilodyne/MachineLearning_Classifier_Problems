...
import xilodyne.util.ScatterPlotter;
...
public class Class_vis {
	...
	public void prettyPicture(ClassifyNB clf, NDArray features_test, double[] predicted) {
		...
		log.logln("\nResultReshape: " + ArrayUtils.print2DArray(boundaryDecisionReshaped));
		...
		log.logln("grade_sig: " + ArrayUtils.print1DArray(grade_sig));
		log.logln("bumpy_sig: " + ArrayUtils.print1DArray(bumpy_sig));
		log.logln("grade_bkg: " + ArrayUtils.print1DArray(grade_bkg));
		log.logln("bumpy_bkg: " + ArrayUtils.print1DArray(bumpy_bkg));
