...
public class Prep_terrain_data {
	...
	public void makeTerrainData(NDArray X_train, NDArray X_test, double[] y_train, double[] y_test) {
		...
		log.logln(G.lD, "\nGrade: " + ArrayUtils.print1DArray(grade));
		log.logln("\nBumpy: " + ArrayUtils.print1DArray(bumpy));
		log.logln("\nError: " + ArrayUtils.print1DArray(error));
		log.logln("\ny: " + ArrayUtils.print1DArray(y));

		for (int loop = 0; loop < StudentMain.size; loop++)
			if (grade[loop] > 0.8 || bumpy[loop] > 0.8)
				y[loop] = 1.0;

		log.logln_noTimestamp("y: " + ArrayUtils.print1DArray(y));
		...
		log.logln("\nX_train: " + X_train);
		log.logln("\nX_test:  " + X_test);
		log.logln("\ny_train: " + ArrayUtils.print1DArray(y_train));
		log.logln("\ny_test: " + ArrayUtils.print1DArray(y_test));
